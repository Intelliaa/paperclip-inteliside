...
import { taskorgConfigSchema, type TaskOrgConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".taskorg", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  if (process.env.TASKORG_CONFIG) return path.resolve(process.env.TASKORG_CONFIG);
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveTaskOrgInstanceId());
}
...
export function readConfig(configPath?: string): TaskOrgConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = taskorgConfigSchema.safeParse(migrated);
  if (!parsed.success) {
