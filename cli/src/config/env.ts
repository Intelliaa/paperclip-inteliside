...
const JWT_SECRET_ENV_KEY = "TASKORG_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by TaskOrg CLI commands",
...
export function resolveTaskOrgEnvFile(configPath?: string): string {
...
export function loadTaskOrgEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeTaskOrgEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readTaskOrgEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeTaskOrgEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeTaskOrgEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readTaskOrgEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeTaskOrgEnvEntries(next, filePath);
  return next;
}
