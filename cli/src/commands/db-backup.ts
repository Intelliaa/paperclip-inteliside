...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@taskorg/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://taskorg:taskorg@127.0.0.1:${port}/taskorg`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" taskorg db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveTaskOrgInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "taskorg";
