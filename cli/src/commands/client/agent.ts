...
import type { Agent } from "@taskorg/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveTaskOrgSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export TASKORG_API_URL='${escaped(input.apiBase)}'`,
    `export TASKORG_COMPANY_ID='${escaped(input.companyId)}'`,
    `export TASKORG_AGENT_ID='${escaped(input.agentId)}'`,
    `export TASKORG_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local TaskOrg skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing TaskOrg skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveTaskOrgSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local TaskOrg skills directory. Expected ./skills in the repo checkout.",
              );
            }
