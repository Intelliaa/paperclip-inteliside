...
import { ApiRequestError, TaskOrgApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to TaskOrg config file")
    .option("-d, --data-dir <path>", "TaskOrg data directory root (isolates state from ~/.taskorg)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the TaskOrg API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.TASKORG_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.TASKORG_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.TASKORG_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set TASKORG_COMPANY_ID, or set context profile companyId via `taskorg context set`.",
    );
  }

  const api = new TaskOrgApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.TASKORG_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.TASKORG_SERVER_PORT || "");
