...
export const DEFAULT_WORKTREE_HOME = "~/.taskorg-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".taskorg");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: TaskOrgConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "taskorg",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    TASKORG_HOME: paths.homeDir,
    TASKORG_INSTANCE_ID: paths.instanceId,
    TASKORG_CONFIG: paths.configPath,
    TASKORG_CONTEXT: paths.contextPath,
    TASKORG_IN_WORKTREE: "true",
    ...(branding?.name ? { TASKORG_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { TASKORG_WORKTREE_COLOR: branding.color } : {}),
  };
}
