...
import { ensureAgentJwtSecret, loadTaskOrgEnvFile, mergeTaskOrgEnvEntries, readTaskOrgEnvEntries, resolveTaskOrgEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { TaskOrgConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printTaskOrgCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.TASKORG_CONFIG;
...
const WORKTREE_NAME_PREFIX = "taskorg-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.TASKORG_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.TASKORG_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromTaskOrgConfig(config: TaskOrgConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromTaskOrgConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.taskorg${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".taskorg", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".taskorg", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.taskorg"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: TaskOrgConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://taskorg:taskorg@127.0.0.1:${port}/taskorg`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: TaskOrgConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.TASKORG_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.TASKORG_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.TASKORG_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.TASKORG_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: TaskOrgConfig;
  targetConfig: TaskOrgConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveTaskOrgEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readTaskOrgEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://taskorg:taskorg@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "taskorg");
    const targetConnectionString = `postgres://taskorg:taskorg@127.0.0.1:${targetHandle.port}/taskorg`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readTaskOrgEnvEntries(resolveTaskOrgEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.TASKORG_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.TASKORG_AGENT_JWT_SECRET);
  mergeTaskOrgEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { TASKORG_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadTaskOrgEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run TaskOrg inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printTaskOrgCliBanner();
  p.intro(pc.bgCyan(pc.black(" taskorg worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printTaskOrgCliBanner();
  p.intro(pc.bgCyan(pc.black(" taskorg worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasTaskOrgConfig: existsSync(path.resolve(worktreePath, ".taskorg", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printTaskOrgCliBanner();
  p.intro(pc.bgCyan(pc.black(" taskorg worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveTaskOrgEnvFile(configPath);
  const envEntries = readTaskOrgEnvEntries(envPath);
  const out = {
    TASKORG_CONFIG: configPath,
    ...(envEntries.TASKORG_HOME ? { TASKORG_HOME: envEntries.TASKORG_HOME } : {}),
    ...(envEntries.TASKORG_INSTANCE_ID ? { TASKORG_INSTANCE_ID: envEntries.TASKORG_INSTANCE_ID } : {}),
    ...(envEntries.TASKORG_CONTEXT ? { TASKORG_CONTEXT: envEntries.TASKORG_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasTaskOrgConfig)
      .map((choice) => path.resolve(choice.worktree, ".taskorg", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readTaskOrgEnvEntries(resolveTaskOrgEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start TaskOrg once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasTaskOrgConfig ? "taskorg" : "no-taskorg-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".taskorg", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".taskorg", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .taskorg/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasTaskOrgConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a TaskOrg worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasTaskOrgConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No TaskOrg worktrees were found. Run `taskorg worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target TaskOrg configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local TaskOrg instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated TaskOrg instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with taskorg- if needed (created at ~/taskorg-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: TASKORG_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: TASKORG_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source TASKORG_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local TaskOrg instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like TaskOrg worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with taskorg- if needed")
