...
type OnboardDefaults = Pick<TaskOrgConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "TASKORG_PUBLIC_URL",
  "DATABASE_URL",
  "TASKORG_DB_BACKUP_ENABLED",
  "TASKORG_DB_BACKUP_INTERVAL_MINUTES",
  "TASKORG_DB_BACKUP_RETENTION_DAYS",
  "TASKORG_DB_BACKUP_DIR",
  "TASKORG_DEPLOYMENT_MODE",
  "TASKORG_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "TASKORG_ALLOWED_HOSTNAMES",
  "TASKORG_AUTH_BASE_URL_MODE",
  "TASKORG_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "TASKORG_STORAGE_PROVIDER",
  "TASKORG_STORAGE_LOCAL_DIR",
  "TASKORG_STORAGE_S3_BUCKET",
  "TASKORG_STORAGE_S3_REGION",
  "TASKORG_STORAGE_S3_ENDPOINT",
  "TASKORG_STORAGE_S3_PREFIX",
  "TASKORG_STORAGE_S3_FORCE_PATH_STYLE",
  "TASKORG_SECRETS_PROVIDER",
  "TASKORG_SECRETS_STRICT_MODE",
  "TASKORG_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const publicUrl =
    process.env.TASKORG_PUBLIC_URL?.trim() ||
    process.env.TASKORG_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.TASKORG_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.TASKORG_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.TASKORG_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.TASKORG_ALLOWED_HOSTNAMES
    ? process.env.TASKORG_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.TASKORG_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.TASKORG_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.TASKORG_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.TASKORG_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.TASKORG_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.TASKORG_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.TASKORG_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.TASKORG_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.TASKORG_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.TASKORG_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.TASKORG_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.TASKORG_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.TASKORG_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.TASKORG_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.TASKORG_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "TASKORG_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<TaskOrgConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printTaskOrgCliBanner();
  p.intro(pc.bgCyan(pc.black(" taskorg onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveTaskOrgInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: TaskOrgConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing TaskOrg install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("taskorg configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("TASKORG_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.TASKORG_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("TASKORG_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("TASKORG_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: TASKORG_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("taskorg run")}`,
        `Reconfigure later: ${pc.cyan("taskorg configure")}`,
        `Diagnose setup: ${pc.cyan("taskorg doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.TASKORG_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing TaskOrg setup is ready.");
    return;
  }
  ...
  let llm: TaskOrgConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@taskorg/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `taskorg doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("taskorg run")}`,
        `Then: ${pc.cyan("taskorg auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
