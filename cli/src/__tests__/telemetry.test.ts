...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.TASKORG_HOME = path.join(root, "home");
    process.env.TASKORG_INSTANCE_ID = "telemetry-test";
