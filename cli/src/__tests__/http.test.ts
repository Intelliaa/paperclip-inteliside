...
import { ApiConnectionError, ApiRequestError, TaskOrgApiClient } from "../client/http.js";

describe("TaskOrgApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new TaskOrgApiClient({
    ...
    expect(headers["x-taskorg-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new TaskOrgApiClient({ apiBase: "http://localhost:3100" });
