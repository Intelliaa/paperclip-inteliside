...
describe("isGithubShorthand", () => {
  it("matches owner/repo/path shorthands", () => {
    expect(isGithubShorthand("taskorg/companies/gstack")).toBe(true);
    expect(isGithubShorthand("taskorg/companies")).toBe(true);
  });
...
describe("normalizeGithubImportSource", () => {
  it("normalizes shorthand imports to canonical GitHub sources", () => {
    expect(normalizeGithubImportSource("taskorg/companies/gstack")).toBe(
      "https://github.com/taskorg/companies?ref=main&path=gstack",
    );
  });

  it("applies --ref to shorthand imports", () => {
    expect(normalizeGithubImportSource("taskorg/companies/gstack", "feature/demo")).toBe(
      "https://github.com/taskorg/companies?ref=feature%2Fdemo&path=gstack",
    );
  });
  ...
  it("applies --ref to existing GitHub tree URLs without losing the package path", () => {
    expect(
      normalizeGithubImportSource(
        "https://github.com/taskorg/companies/tree/main/gstack",
        "release/2026-03-23",
      ),
    ).toBe(
      "https://github.com/taskorg/companies?ref=release%2F2026-03-23&path=gstack",
    );
  });
});
