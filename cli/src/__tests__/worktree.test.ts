...
function buildSourceConfig(): TaskOrgConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("taskorg-pr-432")).toBe(
      path.resolve(os.homedir(), "taskorg-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("taskorg/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://taskorg.example", 3110)).toBe("https://taskorg.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/taskorg-feature",
      homeDir: "/tmp/taskorg-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/taskorg-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/taskorg-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.TASKORG_HOME).toBe(path.resolve("/tmp/taskorg-worktrees"));
    expect(env.TASKORG_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.TASKORG_IN_WORKTREE).toBe("true");
    expect(env.TASKORG_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.TASKORG_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export TASKORG_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-worktree-secrets-"));
    const originalInlineMasterKey = process.env.TASKORG_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.TASKORG_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.TASKORG_SECRETS_MASTER_KEY;
      delete process.env.TASKORG_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.TASKORG_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.TASKORG_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.TASKORG_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          TASKORG_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.TASKORG_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.TASKORG_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".taskorg-worktrees"),
      });

      const envPath = path.join(repoRoot, ".taskorg", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("TASKORG_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("TASKORG_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/TASKORG_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.TASKORG_AGENT_JWT_SECRET;
      } else {
        process.env.TASKORG_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".taskorg-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".taskorg", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local TaskOrg config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".taskorg", "config.json");
    const originalCwd = process.cwd();
    const originalTaskOrgConfig = process.env.TASKORG_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalTaskOrgConfig === undefined) {
        delete process.env.TASKORG_CONFIG;
      } else {
        process.env.TASKORG_CONFIG = originalTaskOrgConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/taskorg",
        targetRepoRoot: "/Users/example/taskorg-pr-432",
        workspaceCwd: "/Users/example/taskorg",
      }),
    ).toBe("/Users/example/taskorg-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/taskorg",
        targetRepoRoot: "/Users/example/taskorg-pr-432",
        workspaceCwd: "/Users/example/taskorg/packages/db",
      }),
    ).toBe("/Users/example/taskorg-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "taskorg-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("taskorg-make-test", {
        seed: false,
        home: path.join(tempRoot, ".taskorg-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".taskorg", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".taskorg", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
