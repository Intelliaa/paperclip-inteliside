...
export class TaskOrgApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-taskorg-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the TaskOrg API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the TaskOrg server is not running, the configured URL is wrong, or the request is being blocked before it reaches TaskOrg.",
    "",
    "Try:",
    "- Start TaskOrg with `pnpm dev` or `pnpm taskorg run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If TaskOrg is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`TASKORG_API_URL\`.`,
  );
  return lines.join("\n");
}
