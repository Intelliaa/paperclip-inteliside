/**
 * esbuild configuration for building the taskorg CLI for npm.
 *
 * Bundles all workspace packages (@taskorg/*) into a single file.
...
// to npm and resolved at runtime (e.g. @taskorg/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@taskorg/")) {
      externals.add(name);
    }
  }
