...
import type { DeploymentExposure, DeploymentMode } from "@taskorg/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "taskorg" &&
    normalized !== "taskorg-create-agent" &&
    normalized !== "taskorg-create-plugin" &&
...
/** Resolve the TaskOrg repo skills directory (built-in / managed skills). */
function resolveTaskOrgSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const taskorgSkillsDir = resolveTaskOrgSkillsDir();

  // Build set of TaskOrg-managed skill names
  const taskorgSkillNames = new Set<string>();
  if (taskorgSkillsDir) {
    try {
      for (const entry of fs.readdirSync(taskorgSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) taskorgSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isTaskOrgManaged: taskorgSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.taskorgApiUrl)) {
    const legacyTaskOrgApiUrl = nonEmptyTrimmedString(input.taskorgApiUrl);
    if (legacyTaskOrgApiUrl) merged.taskorgApiUrl = legacyTaskOrgApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    taskorgApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.taskorgApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawTaskOrgApiUrl =
    typeof defaults.taskorgApiUrl === "string"
      ? defaults.taskorgApiUrl.trim()
      : "";
  if (rawTaskOrgApiUrl) {
    try {
      const parsedTaskOrgApiUrl = new URL(rawTaskOrgApiUrl);
      if (
        parsedTaskOrgApiUrl.protocol !== "http:" &&
        parsedTaskOrgApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_taskorg_api_url_protocol",
          level: "warn",
          message: `taskorgApiUrl must use http:// or https:// (got ${parsedTaskOrgApiUrl.protocol}).`
        });
      } else {
        normalized.taskorgApiUrl = parsedTaskOrgApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_taskorg_api_url_configured",
          level: "info",
          message: `taskorgApiUrl set to ${parsedTaskOrgApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_taskorg_api_url_invalid",
        level: "warn",
        message: `Invalid taskorgApiUrl: ${rawTaskOrgApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your TaskOrg host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "TaskOrg is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm taskorg allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/taskorg";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/taskorg-claimed-api-key.json and load TASKORG_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: taskorgApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the TaskOrg hostname is reachable and allowed via `pnpm taskorg allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this TaskOrg API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/taskorg/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # TaskOrg OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          taskorgApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, TaskOrg generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "taskorgApiUrl": "https://taskorg-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in TaskOrg before key claim is allowed.
    ...
    - ~/.openclaw/workspace/taskorg-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/taskorg-claimed-api-key.json

    And set the TASKORG_API_KEY and TASKORG_API_URL in your environment variables as specified here:
    ...
    Then set TASKORG_API_KEY and TASKORG_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install TaskOrg skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your TASKORG_API_URL to the top of your skill and note the path to your TASKORG_API_URL
    ...
    ${
      ...
      "Ensure TaskOrg is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested TaskOrg base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.taskorgApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm taskorg allowed-hostname <host>
      - then restart TaskOrg and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@taskorg.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "taskorg", path: "/api/skills/taskorg" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "taskorg-create-agent",
          path: "/api/skills/taskorg-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              taskorgApiUrl: req.body.taskorgApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.taskorgApiUrl &&
          !persistedDefaults.taskorgApiUrl
        ) {
          missingPersistedFields.push("taskorgApiUrl");
        }
