...
import { issues, projects, projectWorkspaces } from "@taskorg/db";
import { updateExecutionWorkspaceSchema } from "@taskorg/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before TaskOrg can manage local runtime services" });
      return;
    }
