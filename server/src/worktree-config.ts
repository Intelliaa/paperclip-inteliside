...
import type { TaskOrgConfig } from "@taskorg/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by TaskOrg worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.TASKORG_IN_WORKTREE !== "true") return null;

  const configPath = resolveTaskOrgConfigPath(overrideConfigPath);
  const envPath = resolveTaskOrgEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.TASKORG_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.TASKORG_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.TASKORG_HOME) ??
      nonEmpty(env.TASKORG_WORKTREES_DIR) ??
      "~/.taskorg-worktrees",
  );
...
function writeConfigFile(configPath: string, config: TaskOrgConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".taskorg", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as TaskOrgConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: TaskOrgConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: TaskOrgConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.TASKORG_HOME = context.homeDir;
  process.env.TASKORG_INSTANCE_ID = context.instanceId;
  process.env.TASKORG_CONFIG = context.configPath;
  process.env.TASKORG_CONTEXT = context.contextPath;
  process.env.TASKORG_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as TaskOrgConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    TASKORG_HOME: context.homeDir,
    TASKORG_INSTANCE_ID: context.instanceId,
    TASKORG_CONFIG: context.configPath,
    TASKORG_CONTEXT: context.contextPath,
    TASKORG_IN_WORKTREE: "true",
    TASKORG_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: TaskOrgConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as TaskOrgConfig;
  } catch {
    return;
  }
