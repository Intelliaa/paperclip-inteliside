...
function jwtConfig() {
  const secret = process.env.TASKORG_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.TASKORG_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.TASKORG_AGENT_JWT_ISSUER ?? "taskorg",
    audience: process.env.TASKORG_AGENT_JWT_AUDIENCE ?? "taskorg-api",
  };
}
