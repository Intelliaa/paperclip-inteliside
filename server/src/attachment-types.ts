 ...
 * `TASKORG_ALLOWED_ATTACHMENT_TYPES` environment variable to a
 * comma-separated list of MIME types or wildcard patterns to expand the
 * allowed set.
 *
 * Examples:
 *   TASKORG_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf
 *   TASKORG_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf,text/*
 *
...
const allowedPatterns: string[] = parseAllowedTypes(
  process.env.TASKORG_ALLOWED_ATTACHMENT_TYPES,
);
...
export const MAX_ATTACHMENT_BYTES =
  Number(process.env.TASKORG_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;
