...
describe("agent local JWT", () => {
  const secretEnv = "TASKORG_AGENT_JWT_SECRET";
  const ttlEnv = "TASKORG_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "TASKORG_AGENT_JWT_ISSUER";
  const audienceEnv = "TASKORG_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "taskorg";
    process.env[audienceEnv] = "taskorg-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
