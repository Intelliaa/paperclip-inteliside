...
import { writeTaskOrgSkillSyncPreference } from "@taskorg/adapter-utils/server-utils";
import {
  agents,
...
async function startTempDatabase() {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-feedback-service-"));
  ...
  const adminConnectionString = `postgres://taskorg:taskorg@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "taskorg");
  const connectionString = `postgres://taskorg:taskorg@127.0.0.1:${port}/taskorg`;
...
describe("feedbackService.saveIssueVote", () => {
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-feedback-instructions-"));
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeTaskOrgSkillSyncPreference(
        {
          model: "gpt-5.4",
        ...
        ["taskorg/taskorg/taskorg", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("taskorg-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("taskorg-feedback-bundle-v2");
    ...
    const octoSkill = skillItems?.find((s: Record<string, unknown>) => s.key === "octo/research/public-skill");
    expect(octoSkill?.sourceLocator).toBe("https://github.com/octo/research/tree/main/skills/public-skill");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("taskorg-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("taskorg-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-feedback-opencode-"));
    ...
    vi.stubEnv("TASKORG_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "taskorg_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });
