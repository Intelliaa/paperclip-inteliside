...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@taskorg/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@taskorg/adapter-codex-local/ui";
import { printCodexStreamEvent } from "@taskorg/adapter-codex-local/cli";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use taskorg skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use taskorg skill**" },
    ]);
  });
  ...
  it("parses command execution and file changes", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            id: "item_52",
            type: "file_change",
            changes: [{ path: "/Users/taskorguser/project/ui/src/pages/AgentDetail.tsx", kind: "update" }],
            status: "completed",
          },
        }),
        ts,
    ...
    ).toEqual([
      {
        kind: "system",
        ts,
        text: "file changes: update /Users/taskorguser/project/ui/src/pages/AgentDetail.tsx",
      },
    ]);
  });
