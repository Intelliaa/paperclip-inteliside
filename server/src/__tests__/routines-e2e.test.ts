...
vi.mock("../services/index.js", async () => {
  ...
  const { heartbeatRuns, issues } = await import("@taskorg/db");
...
describeEmbeddedPostgres("routine routes end-to-end", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("taskorg-routines-e2e-");
  ...
  it("runs routines with variable inputs and interpolates the execution issue description", async () => {
    ...
    expect(issue?.description).toBe("Review taskorg for high bugs");
  });
