...
describe("agent instructions service", () => {
  const originalTaskOrgHome = process.env.TASKORG_HOME;
  const originalTaskOrgInstanceId = process.env.TASKORG_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalTaskOrgHome === undefined) delete process.env.TASKORG_HOME;
    else process.env.TASKORG_HOME = originalTaskOrgHome;
    if (originalTaskOrgInstanceId === undefined) delete process.env.TASKORG_INSTANCE_ID;
    else process.env.TASKORG_INSTANCE_ID = originalTaskOrgInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const taskorgHome = await makeTempDir("taskorg-agent-instructions-home-");
    const externalRoot = await makeTempDir("taskorg-agent-instructions-external-");
    cleanupDirs.add(taskorgHome);
    cleanupDirs.add(externalRoot);
    process.env.TASKORG_HOME = taskorgHome;
    process.env.TASKORG_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("taskorg-agent-instructions-new-external-");
    cleanupDirs.add(taskorgHome);
    cleanupDirs.add(externalRoot);
    process.env.TASKORG_HOME = taskorgHome;
    process.env.TASKORG_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("taskorg-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const taskorgHome = await makeTempDir("taskorg-agent-instructions-recover-");
    cleanupDirs.add(taskorgHome);
    process.env.TASKORG_HOME = taskorgHome;
    process.env.TASKORG_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const taskorgHome = await makeTempDir("taskorg-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("taskorg-agent-instructions-stale-root-");
    cleanupDirs.add(taskorgHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const taskorgHome = await makeTempDir("taskorg-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("taskorg-agent-instructions-heal-write-stale-");
    cleanupDirs.add(taskorgHome);
    cleanupDirs.add(staleRoot);
    process.env.TASKORG_HOME = taskorgHome;
    process.env.TASKORG_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const taskorgHome = await makeTempDir("taskorg-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("taskorg-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(taskorgHome);
    cleanupDirs.add(staleRoot);
    process.env.TASKORG_HOME = taskorgHome;
    process.env.TASKORG_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const taskorgHome = await makeTempDir("taskorg-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("taskorg-agent-instructions-partial-root-");
    cleanupDirs.add(taskorgHome);
    cleanupDirs.add(staleRoot);
    process.env.TASKORG_HOME = taskorgHome;
    process.env.TASKORG_INSTANCE_ID = "test-instance";
