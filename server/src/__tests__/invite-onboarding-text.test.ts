...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("TaskOrg OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested TaskOrg base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("taskorgApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.taskorgApiUrl");
    expect(text).toContain("~/.openclaw/workspace/taskorg-claimed-api-key.json");
    expect(text).toContain("TASKORG_API_KEY");
