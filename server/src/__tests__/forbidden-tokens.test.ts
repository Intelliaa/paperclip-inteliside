...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "taskorg", LOGNAME: "taskorg", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "taskorg" }),
      },
    );

    expect(tokens).toEqual(["taskorg", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "taskorg" }, {
        userInfo: () => ({ username: "taskorg" }),
      });

      expect(tokens).toEqual(["taskorg", "custom-token"]);
    } finally {
