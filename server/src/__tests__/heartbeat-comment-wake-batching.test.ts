...
async function startTempDatabase() {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "taskorg-heartbeat-comment-wake-"));
...
describe("heartbeat comment wake batching", () => {
  ...
  it("batches deferred comment wakes and forwards the ordered batch to the next run", async () => {
    ...
    try {
      ...
      const deferredContext = (deferredWake?.payload as Record<string, unknown> | null)?._taskorgWakeContext as
      ...
      expect(secondPayload.taskorg).toMatchObject({
        wake: {
