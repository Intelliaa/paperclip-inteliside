...
import type { QuotaWindow } from "@taskorg/adapter-utils";
...
describe("readClaudeToken", () => {
  ...
  it("returns null when credentials.json does not exist", async () => {
    ...
    process.env.CLAUDE_CONFIG_DIR = "/tmp/__no_such_taskorg_dir__";
    const token = await readClaudeToken();
    expect(token).toBe(null);
  });

  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `taskorg-test-claude-${Date.now()}`);
...
describe("readCodexAuthInfo", () => {
  ...
  it("returns null when auth.json does not exist", async () => {
    process.env.CODEX_HOME = "/tmp/__no_such_taskorg_codex_dir__";
    const result = await readCodexAuthInfo();
    expect(result).toBe(null);
  });

  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `taskorg-test-codex-${Date.now()}`);
    await import("node:fs/promises").then((fs) =>
      fs.mkdir(tmpDir, { recursive: true }).then(() =>
