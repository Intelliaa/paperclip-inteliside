...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "taskorg-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "TaskOrg Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@taskorg.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("taskorg-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `taskorg-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "taskorg-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "taskorg-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "taskorg-close-check",
      baseRef: "main",
      metadata: {
  ...
  it("shows inherited shared project runtime services on shared execution workspaces without duplicating old history", async () => {
    ...
    const reuseKey = `project_workspace:${projectWorkspaceId}:taskorg-dev`;
    ...
    await db.insert(projectWorkspaces).values({
      ...
      metadata: {
        runtimeConfig: {
          ...
          workspaceRuntime: {
            services: [{ name: "taskorg-dev", command: "pnpm dev" }],
          },
        },
      },
    });
