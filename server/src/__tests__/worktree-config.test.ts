...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "taskorg-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const taskorgDir = path.join(worktreeRoot, ".taskorg");
    const configPath = path.join(taskorgDir, "config.json");
    const envPath = path.join(taskorgDir, ".env");
    const sharedRoot = path.join(tempRoot, ".taskorg", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".taskorg-worktrees");

    await fs.mkdir(taskorgDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# TaskOrg environment variables",
        "TASKORG_IN_WORKTREE=true",
        "TASKORG_WORKTREE_NAME=PAP-884-ai-commits-component",
        "TASKORG_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.TASKORG_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.TASKORG_WORKTREES_DIR = isolatedHome;
    delete process.env.TASKORG_HOME;
    delete process.env.TASKORG_INSTANCE_ID;
    delete process.env.TASKORG_CONFIG;
    delete process.env.TASKORG_CONTEXT;
    ...
    expect(repairedEnv).toContain(`TASKORG_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('TASKORG_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`TASKORG_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`TASKORG_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('TASKORG_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.TASKORG_HOME).toBe(isolatedHome);
    expect(process.env.TASKORG_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "taskorg-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# TaskOrg environment variables",
        "TASKORG_IN_WORKTREE=true",
        "TASKORG_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.TASKORG_IN_WORKTREE = "true";
    process.env.TASKORG_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.TASKORG_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "taskorg-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".taskorg-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".taskorg", "worktrees");
    ...
    const taskorgDir = path.join(currentWorktreeRoot, ".taskorg");
    const configPath = path.join(taskorgDir, "config.json");
    const envPath = path.join(taskorgDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".taskorg", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "taskorg-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.TASKORG_IN_WORKTREE = "true";
    process.env.TASKORG_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.TASKORG_HOME = isolatedHome;
    process.env.TASKORG_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.TASKORG_CONFIG = configPath;
