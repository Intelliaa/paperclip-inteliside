...
import { ensureCursorSkillsInjected } from "@taskorg/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing TaskOrg skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("taskorg-cursor-skills-src-");
    const skillsHome = await makeTempDir("taskorg-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "taskorg");
    await createSkillDir(skillsDir, "taskorg-create-agent");
    ...
    const injectedA = path.join(skillsHome, "taskorg");
    const injectedB = path.join(skillsHome, "taskorg-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "taskorg")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "taskorg-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "taskorg"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "taskorg-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("taskorg-cursor-preserve-src-");
    const skillsHome = await makeTempDir("taskorg-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "taskorg");
    ...
    expect((await fs.lstat(path.join(skillsHome, "taskorg-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("taskorg-cursor-fail-src-");
    const skillsHome = await makeTempDir("taskorg-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
