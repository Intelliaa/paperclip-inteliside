...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@taskorg/db";
 ...
 *
 * This implementation delegates to the core TaskOrg domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@taskorg/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
