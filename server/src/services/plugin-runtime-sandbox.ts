...
import type { TaskOrgPluginManifestV1 } from "@taskorg/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __taskorg_exports: module.exports,
      __taskorg_module: module,
      __taskorg_require: requireInSandbox,
      __taskorg_filename: realPath,
      __taskorg_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__taskorg_exports, __taskorg_module, __taskorg_require, __taskorg_filename, __taskorg_dirname)`;
