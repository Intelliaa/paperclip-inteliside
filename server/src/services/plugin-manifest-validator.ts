 ...
 */
import { pluginManifestV1Schema } from "@taskorg/shared";
import type { TaskOrgPluginManifestV1 } from "@taskorg/shared";
import { PLUGIN_API_VERSION } from "@taskorg/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): TaskOrgPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as TaskOrgPluginManifestV1,
        };
      }
