...
import {
  readTaskOrgSkillSyncPreference,
  writeTaskOrgSkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".taskorg.yaml" || normalized === ".taskorg.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const taskorg = isPlainRecord(metadata?.taskorg) ? metadata?.taskorg as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.taskorgSkillKey)
    ?? asString(taskorg?.skillKey)
    ?? asString(taskorg?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "taskorg_bundled") {
    return `taskorg/taskorg/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .taskorg.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .taskorg.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .taskorg.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .taskorg.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; TaskOrg will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .taskorg.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .taskorg.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[taskorgExtensionPath];
  if (selectedFiles.has(taskorgExtensionPath) && typeof extensionEntry === "string") {
    filtered[taskorgExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findTaskOrgExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".taskorg.yaml"] === "string") return ".taskorg.yaml";
  if (typeof files[".taskorg.yml"] === "string") return ".taskorg.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.taskorg.yaml") || entry.endsWith("/.taskorg.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "taskorg_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "taskorg/taskorg",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/Intelliaa/paperclip-inteliside/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "taskorg_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.taskorgSkillKey = skill.key;
  metadata.taskorg = {
    ...(isPlainRecord(metadata.taskorg) ? metadata.taskorg : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const taskorgExtensionPath = findTaskOrgExtensionPath(normalizedFiles);
  const taskorgExtension = taskorgExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, taskorgExtensionPath) ?? "")
    : {};
  const taskorgCompany = isPlainRecord(taskorgExtension.company) ? taskorgExtension.company : {};
  const taskorgSidebar = normalizePortableSidebarOrder(taskorgExtension.sidebar);
  const taskorgAgents = isPlainRecord(taskorgExtension.agents) ? taskorgExtension.agents : {};
  const taskorgProjects = isPlainRecord(taskorgExtension.projects) ? taskorgExtension.projects : {};
  const taskorgTasks = isPlainRecord(taskorgExtension.tasks) ? taskorgExtension.tasks : {};
  const taskorgRoutines = isPlainRecord(taskorgExtension.routines) ? taskorgExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(taskorgCompany.brandColor),
      logoPath: asString(taskorgCompany.logoPath) ?? asString(taskorgCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof taskorgCompany.requireBoardApprovalForNewAgents === "boolean"
          ? taskorgCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof taskorgCompany.feedbackDataSharingEnabled === "boolean"
          ? taskorgCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof taskorgCompany.feedbackDataSharingConsentAt === "string"
          ? taskorgCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(taskorgCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(taskorgCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: taskorgSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(taskorgAgents[slug]) ? taskorgAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(taskorgProjects[slug]) ? taskorgProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(taskorgTasks[slug]) ? taskorgTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(taskorgRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(taskorgRoutines[slug]) ? taskorgRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".taskorg.yaml" ||
          relative === ".taskorg.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const taskorgAgentsOut: Record<string, Record<string, unknown>> = {};
    const taskorgProjectsOut: Record<string, Record<string, unknown>> = {};
    const taskorgTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const taskorgRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readTaskOrgSkillSyncPreference(
        ...
        taskorgAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      taskorgProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      taskorgTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      taskorgRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const taskorgExtensionPath = ".taskorg.yaml";
    const taskorgAgents = Object.fromEntries(
      Object.entries(taskorgAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const taskorgProjects = Object.fromEntries(
      Object.entries(taskorgProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const taskorgTasks = Object.fromEntries(
      Object.entries(taskorgTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const taskorgRoutines = Object.fromEntries(
      Object.entries(taskorgRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[taskorgExtensionPath] = buildYamlFile(
      {
        schema: "taskorg/v1",
        ...
        agents: Object.keys(taskorgAgents).length > 0 ? taskorgAgents : undefined,
        projects: Object.keys(taskorgProjects).length > 0 ? taskorgProjects : undefined,
        tasks: Object.keys(taskorgTasks).length > 0 ? taskorgTasks : undefined,
        routines: Object.keys(taskorgRoutines).length > 0 ? taskorgRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, taskorgExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeTaskOrgSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
