...
import { readTaskOrgSkillSyncPreference } from "@taskorg/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@taskorg/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@taskorg/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@taskorg/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveTaskOrgInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "taskorg-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "taskorg-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "taskorg-feedback-v1";
const FEEDBACK_DESTINATION = "taskorg_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "taskorg_run" &&
    file.source !== "taskorg_run_events" &&
    file.source !== "taskorg_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.TASKORG_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readTaskOrgSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let taskorgRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      taskorgRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "taskorg/run.json",
        contentType: "application/json",
        source: "taskorg_run",
        contents: `${JSON.stringify(taskorgRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.taskorgRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "taskorg/run-events.json",
        contentType: "application/json",
        source: "taskorg_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "taskorg/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "taskorg_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.taskorgRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
