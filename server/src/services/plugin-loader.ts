 ...
 *    (`~/.taskorg/plugins/`) and `node_modules` for packages matching
 *    the `taskorg-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published TaskOrg plugins.
 * Packages matching this pattern are considered TaskOrg plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "taskorg-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: TaskOrgPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.taskorg/plugins/ local directory
  | "npm"               // npm packages matching taskorg-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: TaskOrgPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the taskorg-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "taskorg-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: TaskOrgPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover TaskOrg plugins installed as npm packages in the current
   * Node.js environment matching the "taskorg-plugin-*" naming convention.
   *
   ...
   * the "taskorgPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a TaskOrg plugin.
   * Throws if the package is a TaskOrg plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<TaskOrgPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: TaskOrgPluginManifestV1;
    newManifest: TaskOrgPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the TaskOrg plugin naming convention.
 * Accepts both the "taskorg-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @taskorg/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "taskorgPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const taskorgPlugin = pkgJson["taskorgPlugin"];
  if (
    taskorgPlugin !== null &&
    typeof taskorgPlugin === "object" &&
    !Array.isArray(taskorgPlugin)
  ) {
    const manifestRelPath = (taskorgPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: TaskOrgPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumTaskOrgVersion;
}
 ...
 *   packageName: "taskorg-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: TaskOrgPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as TaskOrgPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a TaskOrg plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<TaskOrgPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a TaskOrg plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasTaskOrgPlugin = "taskorgPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasTaskOrgPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<TaskOrgPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: TaskOrgPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @taskorg/shared exports). Run those workers through
