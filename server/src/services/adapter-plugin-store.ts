 ...
 * ~/.taskorg/adapter-plugins.json. This is the source of truth for which
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-taskorg-adapter") */
  packageName: string;
...
const TASKORG_DIR = path.join(os.homedir(), ".taskorg");
const ADAPTER_PLUGINS_DIR = path.join(TASKORG_DIR, "adapter-plugins");
const ADAPTER_PLUGINS_STORE_PATH = path.join(TASKORG_DIR, "adapter-plugins.json");
const ADAPTER_SETTINGS_PATH = path.join(TASKORG_DIR, "adapter-settings.json");
...
function ensureDirs(): void {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "taskorg-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for TaskOrg external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
}
