...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@taskorg/db";
import type { DeploymentMode } from "@taskorg/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  taskorgUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).taskorgUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.taskorgUpgradeContext = context;
