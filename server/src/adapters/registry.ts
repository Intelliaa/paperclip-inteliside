...
import { getAdapterSessionManagement } from "@taskorg/adapter-utils";
import {
...
} from "@taskorg/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@taskorg/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@taskorg/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@taskorg/adapter-cursor-local";
import {
...
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@taskorg/adapter-gemini-local";
import {
...
} from "@taskorg/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@taskorg/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@taskorg/adapter-openclaw-gateway";
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[taskorg] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[taskorg] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[taskorg] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[taskorg] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
