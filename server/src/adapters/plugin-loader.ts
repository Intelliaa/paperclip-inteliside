...
function extractUiParserSource(
  ...
  const contractVersion = pkg.taskorg?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no taskorg.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
