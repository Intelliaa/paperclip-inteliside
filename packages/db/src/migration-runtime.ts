...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://taskorg:taskorg@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "taskorg");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://taskorg:taskorg@127.0.0.1:${preferredPort}/taskorg`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://taskorg:taskorg@127.0.0.1:${port}/taskorg`,
  ...
  const adminConnectionString = `postgres://taskorg:taskorg@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "taskorg");

  return {
    connectionString: `postgres://taskorg:taskorg@127.0.0.1:${selectedPort}/taskorg`,
