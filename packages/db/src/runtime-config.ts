...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "taskorg-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveTaskOrgHomeDir(): string {
  const envHome = process.env.TASKORG_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".taskorg");
}

function resolveTaskOrgInstanceId(): string {
  const raw = process.env.TASKORG_INSTANCE_ID?.trim() || DEFAULT_INSTANCE_ID;
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveTaskOrgHomeDir(), "instances", resolveTaskOrgInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".taskorg", CONFIG_BASENAME);
...
function resolveTaskOrgConfigPath(): string {
  if (process.env.TASKORG_CONFIG?.trim()) {
    return path.resolve(process.env.TASKORG_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveTaskOrgEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveTaskOrgEnvPath(configPath);
