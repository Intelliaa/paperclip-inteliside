...
Official TypeScript SDK for TaskOrg plugin authors.

- **Worker SDK:** `@taskorg/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@taskorg/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@taskorg/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@taskorg/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@taskorg/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@taskorg/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@taskorg/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@taskorg/plugin-sdk/ui/hooks` | Hooks only |
| `@taskorg/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@taskorg/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@taskorg/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@taskorg/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@taskorg/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@taskorg/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main TaskOrg app. They can call ordinary TaskOrg HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the TaskOrg instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@taskorg/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@taskorg/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core TaskOrg information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@taskorg/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@taskorg/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@taskorg/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@taskorg/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@taskorg/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@taskorg/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@taskorg/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@taskorg/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@taskorg/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@taskorg/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@taskorg/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@taskorg/plugin-sdk/bundlers";
...
```bash
taskorg-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@taskorg/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__taskorg__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__taskorg__/events` streams `reload` SSE events on UI build changes
