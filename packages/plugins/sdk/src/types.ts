/**
 * Core types for the TaskOrg plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@taskorg/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @taskorg/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of TaskOrg object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the TaskOrg UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit TaskOrg domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core TaskOrg domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the TaskOrg secret provider
...
export interface PluginSecretsClient {
   ...
   * in the TaskOrg secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@taskorg/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@taskorg/plugin-sdk";
 *
