...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".taskorg-sdk");
...
/**
 * Generate a complete TaskOrg plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A TaskOrg plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "taskorg-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["taskorg", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@taskorg/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@taskorg/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@taskorg/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@taskorg/plugin-sdk\` and \`@taskorg/shared\` from a local TaskOrg checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.taskorg-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-taskorg-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <taskorg-sdk-path>]");
    process.exit(1);
  }
