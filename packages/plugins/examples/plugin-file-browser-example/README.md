...
Example TaskOrg plugin that demonstrates:
...
```bash
pnpm --filter @taskorg/plugin-file-browser-example build
pnpm taskorg plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm taskorg plugin uninstall taskorg-file-browser-example --force
```
...
- Optional: use `taskorg-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
