...
type TaskOrgSprite = (typeof TASKORG_SPRITES)[number];
...
function spriteSize(sprite: TaskOrgSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = TASKORG_SPRITES[(baseCol + baseRow) % TASKORG_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = TASKORG_SPRITES[Math.floor(Math.random() * TASKORG_SPRITES.length)]!;
