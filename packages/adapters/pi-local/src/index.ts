...
Use when:
- You want TaskOrg to run Pi (the AI coding agent) locally as the agent runtime
...
- TaskOrg requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/taskorgs/ and resumed with --session.
