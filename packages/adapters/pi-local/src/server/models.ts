...
import type { AdapterModel } from "@taskorg/adapter-utils";
import { asString, runChildProcess } from "@taskorg/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.TASKORG_PI_COMMAND === "string" &&
    process.env.TASKORG_PI_COMMAND.trim().length > 0
      ? process.env.TASKORG_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["TASKORG_", "npm_", "NPM_"] as const;
