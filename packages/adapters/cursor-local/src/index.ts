...
Use when:
- You want TaskOrg to run Cursor Agent CLI locally as the agent runtime
...
- TaskOrg auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$taskorg" and related skills on local runs.
- TaskOrg auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
