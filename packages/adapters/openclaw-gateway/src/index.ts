...
Use when:
- You want TaskOrg to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the TaskOrg server.
...
- taskorgApiUrl (string, optional): absolute TaskOrg base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default taskorg)

Standard outbound payload additions:
- taskorg (object): standardized TaskOrg context added to every gateway agent request
- taskorg.workspace (object, optional): resolved execution workspace for this run
- taskorg.workspaces (array, optional): additional workspace hints TaskOrg exposed to the run
- taskorg.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
