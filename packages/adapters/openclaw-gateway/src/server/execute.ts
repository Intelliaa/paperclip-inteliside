...
const DEFAULT_CLIENT_VERSION = "taskorg";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "taskorg";
  if (input.strategy === "run") return `taskorg:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `taskorg:issue:${input.issueId}`;
  return fallback;
}
...
function resolveTaskOrgApiUrlOverride(value: unknown): string | null {
...
function buildTaskOrgEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const taskorgApiUrlOverride = resolveTaskOrgApiUrlOverride(ctx.config.taskorgApiUrl);
  const taskorgEnv: Record<string, string> = {
    ...buildTaskOrgEnv(ctx.agent),
    TASKORG_RUN_ID: ctx.runId,
  };

  if (taskorgApiUrlOverride) {
    taskorgEnv.TASKORG_API_URL = taskorgApiUrlOverride;
  }
  if (wakePayload.taskId) taskorgEnv.TASKORG_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) taskorgEnv.TASKORG_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) taskorgEnv.TASKORG_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) taskorgEnv.TASKORG_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) taskorgEnv.TASKORG_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    taskorgEnv.TASKORG_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  taskorgEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/taskorg-claimed-api-key.json";
  const orderedKeys = [
    "TASKORG_RUN_ID",
    "TASKORG_AGENT_ID",
    "TASKORG_COMPANY_ID",
  ...
  for (const key of orderedKeys) {
    const value = taskorgEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = taskorgEnv.TASKORG_API_URL ?? "<set TASKORG_API_URL>";

  const lines = [
    "TaskOrg wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `TASKORG_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load TASKORG_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $TASKORG_API_KEY on every API call.",
    "- Use X-TaskOrg-Run-Id: $TASKORG_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: TASKORG_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$TASKORG_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$TASKORG_COMPANY_ID/issues?assigneeAgentId=$TASKORG_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardTaskOrgPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  taskorgEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateTaskOrg = parseObject(payloadTemplate.taskorg);
  const workspace = asRecord(ctx.context.taskorgWorkspace);
  const workspaces = Array.isArray(ctx.context.taskorgWorkspaces)
    ? ctx.context.taskorgWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.taskorgRuntimeServiceIntents)
    ? ctx.context.taskorgRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardTaskOrg: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: taskorgEnv.TASKORG_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.taskorgWake);
  if (Object.keys(structuredWake).length > 0) {
    standardTaskOrg.wake = structuredWake;
  }

  if (workspace) {
    standardTaskOrg.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardTaskOrg.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardTaskOrg.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "taskorg-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const taskorgEnv = buildTaskOrgEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderTaskOrgWakePrompt(ctx.context.taskorgWake);
  const structuredWakeJson = stringifyTaskOrgWakePayload(ctx.context.taskorgWake);
  ...
  const taskorgPayload = buildStandardTaskOrgPayload(ctx, wakePayload, taskorgEnv, payloadTemplate);
  ...
  agentParams.taskorg = taskorgPayload;
