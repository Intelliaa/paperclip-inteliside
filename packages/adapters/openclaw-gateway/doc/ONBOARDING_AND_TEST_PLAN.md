...
## Scope
This plan is now **gateway-only**. TaskOrg supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses TaskOrg skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a TaskOrg task.
...
```bash
pnpm --filter @taskorg/server test:run -- openclaw-gateway
pnpm --filter @taskorg/server typecheck
pnpm --filter @taskorg/ui typecheck
pnpm --filter taskorg typecheck
```
