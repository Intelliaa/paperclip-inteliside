...
description: >
  Crea nuevos agentes en TaskOrg con contratación consciente de gobernanza. Úsalo cuando necesites
...
# Skill de Creación de Agentes de TaskOrg
...
```sh
curl -sS "$TASKORG_API_URL/api/agents/me" \
  -H "Authorization: Bearer $TASKORG_API_KEY"
```

2. Descubre la documentación de configuración de adapter disponible para esta instancia de TaskOrg.

```sh
curl -sS "$TASKORG_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $TASKORG_API_KEY"
```
...
```sh
curl -sS "$TASKORG_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $TASKORG_API_KEY"
```
...
```sh
curl -sS "$TASKORG_API_URL/api/companies/$TASKORG_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $TASKORG_API_KEY"
```
...
```sh
curl -sS "$TASKORG_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $TASKORG_API_KEY"
```
...
- cuando el board apruebe, serás despertado con `TASKORG_APPROVAL_ID`; lee los issues vinculados y cierra/comenta el seguimiento

```sh
curl -sS "$TASKORG_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $TASKORG_API_KEY"

curl -sS -X POST "$TASKORG_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $TASKORG_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$TASKORG_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $TASKORG_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$TASKORG_API_URL/api/approvals/$TASKORG_APPROVAL_ID" \
  -H "Authorization: Bearer $TASKORG_API_KEY"

curl -sS "$TASKORG_API_URL/api/approvals/$TASKORG_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $TASKORG_API_KEY"
```
...
- si el rol necesita skills, asegúrate de que ya existan en la biblioteca de la empresa o instálalos primero usando el flujo de skills de empresa de TaskOrg
...
`skills/taskorg-create-agent/references/api-reference.md`
