...
- Disparar externamente: `POST /api/routine-triggers/public/{publicId}/fire`
  ...
  - HMAC: encabezados `X-TaskOrg-Signature` + `X-TaskOrg-Timestamp`
...
Usa las tablas genéricas de endpoints de API en `skills/taskorg/references/api-reference.md` cuando necesites una referencia completa entre dominios. Usa este archivo cuando necesites comportamiento específico de routines, forma del payload o detalles de políticas.
