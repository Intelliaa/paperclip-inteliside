...
Maintainer runbook for shipping TaskOrg across npm, GitHub, and the website-facing changelog surface.
...
TaskOrg uses calendar versions that still fit semver syntax:
...
2. **npm** — `taskorg` and public workspace packages are published
...
```bash
npx taskorg@canary onboard
# or
npx taskorg@canary onboard --data-dir "$(mktemp -d /tmp/taskorg-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/Intelliaa/paperclip-inteliside/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for TaskOrg. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
TASKORG_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable TASKORG_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f taskorg_version=canary
gh workflow run release-smoke.yml -f taskorg_version=latest
```

Minimum checks:

- `npx taskorg@canary onboard` installs
