...
1. Start TaskOrg in auth mode.
```bash
cd <taskorg-repo-root>
...
3. In TaskOrg UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in TaskOrg UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from TaskOrg invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in TaskOrg with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in TaskOrg UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.taskorg-openclaw.override.yml logs -f openclaw-gateway
```
