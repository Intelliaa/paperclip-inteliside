...
Low-level reference for how TaskOrg packages are prepared and published to npm.
...
TaskOrg no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `taskorg`, imports code from workspace packages such as:
...
### Manual first publish for `@taskorg/ui`
...
npm view @taskorg/ui version

# make sure the dist payload is fresh
pnpm --filter @taskorg/ui build
...
- If `npm view @taskorg/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@taskorg/ui`, the `taskorg` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
TaskOrg uses calendar versions:
