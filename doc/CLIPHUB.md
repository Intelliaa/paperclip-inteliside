...
ClipHub is the public registry where people share, discover, and download TaskOrg company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to TaskOrg what a package registry is to a programming language. TaskOrg already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in TaskOrg — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own TaskOrg instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the TaskOrg spec. It contains:
...
**Install (fresh start):**
```
taskorg install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local TaskOrg instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within TaskOrg, export your company as a template, then publish:

```
taskorg export --template my-company
taskorg publish cliphub my-company
```
...
ClipHub is a **separate service** from TaskOrg itself. TaskOrg is self-hosted; ClipHub is a hosted registry that TaskOrg instances talk to.
...
| **TaskOrg CLI** | `taskorg install`, `taskorg publish`, `taskorg cliphub sync` — built into TaskOrg |
| **TaskOrg UI** | "Browse ClipHub" panel in the TaskOrg web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with TaskOrg) |
| Backend | TypeScript + Hono (consistent with TaskOrg) |
| Database | PostgreSQL |
...
4. Run `taskorg install cliphub:acme/lean-saas-shop`
5. TaskOrg creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in TaskOrg until it works well
2. Export: `taskorg export --template my-agency`
3. Publish: `taskorg publish cliphub my-agency`
...
3. Install just that agent: `taskorg install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to TaskOrg

ClipHub is **not required** to use TaskOrg. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to TaskOrg what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `taskorg install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in TaskOrg web UI
- [ ] `taskorg cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with TaskOrg's own philosophy)
