...
Audience: Operators setting up and running agents in TaskOrg

## 1. What this system does

Agents in TaskOrg do not run continuously.  
...
For `claude_local` and `codex_local`, TaskOrg assumes the CLI is already installed and authenticated on the host machine.
...
TaskOrg stores resumable session state per `(agent, taskKey, adapterType)`.
...
TaskOrg pushes runtime/activity updates to the browser in real time.
