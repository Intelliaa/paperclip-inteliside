...
This spec defines how TaskOrg actually runs agents while staying runtime-agnostic.
...
1. TaskOrg is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, TaskOrg manages lifecycle, and on exit TaskOrg parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `taskorg.skill` (shared TaskOrg skill text block)
...
2. Preferred transport is env vars (`TASKORG_*`) injected at runtime.
