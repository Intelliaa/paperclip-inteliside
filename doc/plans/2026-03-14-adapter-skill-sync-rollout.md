...
This document defines the rollout plan for adapter-wide skill support in TaskOrg.
...
- TaskOrg stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

TaskOrg currently has these adapters:
...
1. Desired skills live in TaskOrg for every adapter.
...
These adapters have a stable local skills directory that TaskOrg can read and manage.
...
These adapters do not have a meaningful TaskOrg-owned persistent install state.
...
- desired skills stored in TaskOrg
...
- runtime already injects TaskOrg skills into `~/.cursor/skills`
...
- runtime already injects TaskOrg skills into `~/.gemini/skills`
...
- runtime already injects TaskOrg skills into `~/.pi/agent/skills`
...
- TaskOrg can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects TaskOrg skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- TaskOrg may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed TaskOrg skills that are clearly marked as TaskOrg-managed
...
- TaskOrg stores desired skills only
...
- avoid destructive wording unless TaskOrg can prove a skill is TaskOrg-managed
...
That gets TaskOrg from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
