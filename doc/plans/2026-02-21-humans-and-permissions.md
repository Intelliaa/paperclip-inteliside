...
- local mode that still feels instant (`npx taskorg run` and go)
...
2. CLI command `pnpm taskorg auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm taskorg onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm taskorg onboard`).
...
- expose key instance settings in API + CLI (`taskorg configure` / `taskorg onboard`)
...
7. `pnpm taskorg onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
