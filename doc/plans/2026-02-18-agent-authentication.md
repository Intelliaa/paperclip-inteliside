...
Agents need API keys to authenticate with TaskOrg. The current approach
...
**Trust model:** The adapter process runs on the same machine as the TaskOrg
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** TaskOrg generates a token and passes it directly to the agent
...
`TASKORG_API_URL`, `TASKORG_AGENT_ID`, `TASKORG_COMPANY_ID`. We need to
add a `TASKORG_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a TaskOrg CLI
...
```
taskorg auth login
# Opens browser -> user confirms -> token stored at ~/.taskorg/credentials
```
...
managed by the TaskOrg server itself.
...
1. A company admin (user or agent) generates an **invite URL** from TaskOrg.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The TaskOrg SKILL.md (or a link to it)
   - What information TaskOrg needs from the agent (e.g. webhook URL, adapter
...
5. TaskOrg stores the pending registration.
...
7. On approval, TaskOrg provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** TaskOrg issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.taskorg.ing/skills/taskorg/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How TaskOrg should send you heartbeats",
...
1. Generate a TaskOrg invite link for the company.
...
4. A TaskOrg company member approves the new agent.
5. TaskOrg begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `TASKORG_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `taskorg auth login` for developer-managed remote agents.                                      |
