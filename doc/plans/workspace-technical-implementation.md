...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/taskorg-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `TASKORG_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `TASKORG_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - TaskOrg may create host-local worktrees, processes, and paths
- `cloud`
  - TaskOrg should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `taskorg | github | vercel | s3 | custom`
...
- use `TASKORG_EXECUTION_TOPOLOGY` as a defaulting guardrail
