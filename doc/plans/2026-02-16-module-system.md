...
TaskOrg's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends TaskOrg's API, UI, and data model | Yes |
...
modules/
  observability/
    taskorg.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@taskorg/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@taskorg/core";
...
```
1. Scan modules/ directory for taskorg.module.json manifests
...
```jsonc
// taskorg.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@taskorg/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@taskorg/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:taskorg/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.taskorg.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm taskorg store list                    # browse available modules and templates
pnpm taskorg store install <module-id>     # install a module
pnpm taskorg store import <template-id>    # import a company template
pnpm taskorg store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one TaskOrg instance | `server:started` |
...
4. **Module config** — `taskorg.config.json` with per-module config, env var interpolation
...
10. **`@taskorg/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
