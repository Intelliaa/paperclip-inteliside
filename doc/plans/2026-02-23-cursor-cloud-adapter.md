...
This document defines the V1 design for a TaskOrg adapter that integrates with
...
4. synthesized stdout events for TaskOrg UI/CLI
...
3. **Agent callback auth to TaskOrg** uses a bootstrap exchange flow (no long-lived TaskOrg key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from TaskOrg endpoints, not full SKILL.md prompt injection.
...
- `taskorgPublicUrl` (optional override; else `TASKORG_PUBLIC_URL` env)
...
## TaskOrg Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `TASKORG_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.taskorgPublicUrl`
2. `process.env.TASKORG_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived TaskOrg credentials in prompt text.

Flow:

1. Before launch/follow-up, TaskOrg mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. TaskOrg validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all TaskOrg API calls.
...
1. Prompt includes a compact instruction to fetch skills from TaskOrg.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/taskorg`
   - `GET /api/skills/taskorg-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <taskorgPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active TaskOrg run context.
...
5. `taskorgPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from TaskOrg skill endpoints |
| TaskOrg auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from TaskOrg context.
3. Add richer PR metadata surfacing in TaskOrg UI.
