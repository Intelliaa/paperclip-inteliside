...
- New config/env settings:
  - `TASKORG_AGENT_JWT_SECRET`
  - `TASKORG_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `TASKORG_AGENT_JWT_ISSUER` (default: `taskorg`)
  - `TASKORG_AGENT_JWT_AUDIENCE` (default: `taskorg-api`)
...
1. In:
   ...
   inject `TASKORG_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `TASKORG_API_KEY`, do not overwrite it.
- Continue injecting:
  - `TASKORG_AGENT_ID`
  - `TASKORG_COMPANY_ID`
  - `TASKORG_API_URL`
...
- Local adapters authenticate without manual `TASKORG_API_KEY` config.
