...
TaskOrg already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model TaskOrg should implement next.

## Product Goals

TaskOrg should let operators:
...
For the next implementation pass, TaskOrg should enforce these defaults:
...
TaskOrg should separate subscription-included usage from billed spend:
...
TaskOrg should have two threshold classes:
...
TaskOrg must know that a project is budget-paused and enforce it.
...
TaskOrg must also block execution before new work starts.
