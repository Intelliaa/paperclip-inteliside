...
This document defines the next-stage plan for TaskOrg company import/export.

The core shift is:

- move from a TaskOrg-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside TaskOrg.
...
TaskOrg already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just TaskOrg
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `taskorg.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.taskorg.yaml` sidecar for high-fidelity TaskOrg-specific details
...
11. adapter-aware skill sync surfaces so TaskOrg can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point TaskOrg at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a TaskOrg sidecar.
...
TaskOrg must not redefine `SKILL.md`.
...
- TaskOrg-specific fidelity lives in `.taskorg.yaml`
- TaskOrg may resolve and install `SKILL.md` packages, but it must not require a TaskOrg-only skill format
...
### 5.4 Base Package Vs TaskOrg Extension
...
- base package:
  ...
  - no TaskOrg-only runtime fields by default
- TaskOrg extension:
  - `.taskorg.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by TaskOrg tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`taskorg.manifest.json` is not part of the future package direction.
...
TaskOrg import/export should support these entity kinds:
...
- TaskOrg can treat “this set of agents came from team package X” as the imported-team model
...
TaskOrg should support source references in package metadata with:
...
TaskOrg should define a new adapter surface area around skills:
...
Planned TaskOrg behavior:

- if an adapter supports read, TaskOrg should show current skills in the UI
- if an adapter supports write, TaskOrg should let the user enable/disable imported skills
- if an adapter supports sync, TaskOrg should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.taskorg.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.taskorg.yaml` alongside the base package
...
## 10. Storage Model Inside TaskOrg
...
TaskOrg should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `taskorg company export <company-id> --out <path>`
- `taskorg company import <path-or-url> --dry-run`
- `taskorg company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in TaskOrg storage, or only referenced at import time?
...
3. Should TaskOrg support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps TaskOrg aligned with:
