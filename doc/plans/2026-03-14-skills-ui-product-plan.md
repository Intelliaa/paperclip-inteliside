...
This document defines the product and UI plan for skill management in TaskOrg.
...
- TaskOrg company import/export can include skills as package content
...
3. The UI must always tell the truth about what TaskOrg knows:
   - desired state in TaskOrg
...
TaskOrg should treat skills at two scopes:
...
- created locally inside TaskOrg later
...
7. “Understand what TaskOrg knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a TaskOrg-specific package layout
...
This matters because Codex and similar adapters may already have local skills that TaskOrg did not install.

These should be clearly marked:

- external
- not managed by TaskOrg
...
- `desired_only`: TaskOrg wants it, adapter does not show it yet
- `external`: adapter has it but TaskOrg does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, TaskOrg only tracks desired state
...
- TaskOrg can still track desired skills
...
- TaskOrg can see adapter skills
...
- `.taskorg.yaml` may add TaskOrg-specific fidelity, but should not replace the base shortname association model
...
- `.taskorg.yaml` may add TaskOrg-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
TaskOrg should have a company-scoped skill model or managed package model representing:
...
TaskOrg should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too TaskOrg-specific.
...
That gives TaskOrg one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
