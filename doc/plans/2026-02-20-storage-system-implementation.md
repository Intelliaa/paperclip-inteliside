...
Add a single storage subsystem for TaskOrg that supports:
...
- API serves attachment bytes through authenticated TaskOrg endpoints.
...
- Default local path is under instance root: `~/.taskorg/instances/<instanceId>/data/storage`.
...
- `taskorg onboard` writes a valid `storage` config block by default.
- `taskorg configure --section storage` can switch between local and s3 modes.
...
- `taskorg doctor` reports actionable storage status.
