...
`PAP-447` asks how TaskOrg should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same TaskOrg instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into TaskOrg:

- some operators use TaskOrg to manage TaskOrg and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as TaskOrg
- Claude and Codex expose different built-in affordances, so TaskOrg should not overfit to one tool

## Core Product Decision

TaskOrg should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by TaskOrg
...
- TaskOrg should not assume that every adapter can see or use a host filesystem path directly
...
If TaskOrg makes worktrees agent-first, it will blur:
...
- TaskOrg core resolves and records execution workspace state
...
That gives TaskOrg room to support:
...
- TaskOrg handles workspace runtime services under the hood using project/workspace policy
...
Once TaskOrg is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that TaskOrg cannot infer
...
- when issue moves to done, should TaskOrg auto-commit?
- should it auto-push?
...
TaskOrg already has the right foundation for a project-first model.
...
For server-spawned local adapters, TaskOrg already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that TaskOrg should mirror directly

Implication:

- for `codex_local`, TaskOrg should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but TaskOrg should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, TaskOrg should separate:
...
TaskOrg should be able to express intentions such as:
...
These are not specific to TaskOrg. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".taskorg/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a TaskOrg-specific `devServer` flag.
...
TaskOrg should distinguish between:
...
TaskOrg should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, TaskOrg should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to TaskOrg
...
For runtime services specifically, TaskOrg should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as TaskOrg-only dev-server flags
...
3. Inject resolved execution workspace details into `context.taskorgWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / TaskOrg instance can be reused through env/config injection when appropriate
...
- TaskOrg can decide whether to reuse or start a fresh service deterministically
...
3. Test against TaskOrg-on-TaskOrg workflows first.
...
7. Workspace runtime services are modeled generically, not as TaskOrg-specific dev-server toggles.
