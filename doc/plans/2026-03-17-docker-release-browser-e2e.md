...
Today release smoke testing for published TaskOrg packages is manual and shell-driven:
...
- runs `taskorg@${TASKORG_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@taskorg.local`
- password: `taskorg-smoke-password`
...
1. A published `taskorg@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `taskorg@latest` Docker install can be smoke-tested by Playwright in CI.
