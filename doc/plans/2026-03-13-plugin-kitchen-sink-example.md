...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented TaskOrg plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside TaskOrg.
...
- package: `@taskorg/plugin-kitchen-sink-example`
- plugin id: `taskorg.kitchen-sink-example` or `taskorg-kitchen-sink-example`

Recommendation: use `taskorg-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core TaskOrg event such as `issue.created`
...
Recommendation: default to a TaskOrg-local endpoint or a stable public echo endpoint to avoid flaky docs.
