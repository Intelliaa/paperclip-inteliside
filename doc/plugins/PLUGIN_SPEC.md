# TaskOrg Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for TaskOrg's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main TaskOrg app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary TaskOrg HTTP APIs directly.
...
TaskOrg plugin design is based on the following assumptions:

1. TaskOrg is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core TaskOrg business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by TaskOrg core.
...
2. Let plugins add major capabilities without editing TaskOrg core.
...
4. Depend on project-local plugin folders such as `.taskorg/plugins`.
...
The single TaskOrg deployment an operator installs and controls.

### 5.2 Company

A first-class TaskOrg business object inside the instance.
...
A trusted in-process extension loaded directly by TaskOrg core.
...
An installable instance-wide extension package loaded through the TaskOrg plugin runtime.
...
TaskOrg has two extension classes.
...
TaskOrg already has a concrete workspace model:
...
Plugins live under the TaskOrg instance directory.

Suggested layout:

- `~/.taskorg/instances/default/plugins/package.json`
- `~/.taskorg/instances/default/plugins/node_modules/`
- `~/.taskorg/instances/default/plugins/.cache/`
- `~/.taskorg/instances/default/data/plugins/<plugin-id>/`
...
TaskOrg should add CLI commands:

- `pnpm taskorg plugin list`
- `pnpm taskorg plugin install <package[@version]>`
- `pnpm taskorg plugin uninstall <plugin-id>`
- `pnpm taskorg plugin upgrade <plugin-id> [version]`
- `pnpm taskorg plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@taskorg/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface TaskOrgPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that TaskOrg agents can use during runs.
...
- TaskOrg server starts one worker process per installed plugin
...
Receives one typed TaskOrg domain event.
...
export function definePlugin(definition: PluginDefinition): TaskOrgPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: TaskOrgPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@taskorg/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@taskorg/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@taskorg/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@taskorg/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@taskorg/plugin-sdk/ui`
...
The `@taskorg/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the TaskOrg secret provider system rather than a plain text input
...
1. Core TaskOrg data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing TaskOrg objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual TaskOrg product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if TaskOrg later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing TaskOrg secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm taskorg plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the TaskOrg server. This is a normative requirement, not optional.
...
### 27.1 `@taskorg/plugin-test-harness`
...
```ts
import { createTestHarness } from "@taskorg/plugin-test-harness";
...
For developing a plugin against a running TaskOrg instance:

- The operator installs the plugin from a local path: `pnpm taskorg plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-taskorg-plugin`) that scaffolds:

- `package.json` with correct `taskorgPlugin` keys
...
- `@taskorg/plugin-workspace-files`
- `@taskorg/plugin-terminal`
- `@taskorg/plugin-git`
- `@taskorg/plugin-linear`
- `@taskorg/plugin-github-issues`
- `@taskorg/plugin-grafana`
- `@taskorg/plugin-runtime-processes`
- `@taskorg/plugin-stripe`
...
- `@taskorg/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@taskorg/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@taskorg/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@taskorg/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@taskorg/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@taskorg/plugin-sdk/ui`
...
- `@taskorg/plugin-test-harness`
- `create-taskorg-plugin` starter template
...
TaskOrg should not implement a generic in-process hook bag modeled directly after local coding tools.

TaskOrg should implement:
...
That is the complete target design for the TaskOrg plugin system.
