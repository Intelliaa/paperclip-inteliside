...
This guide describes the current, implemented way to create a TaskOrg plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main TaskOrg app.
...
node packages/plugins/create-taskorg-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the TaskOrg repo:

```bash
pnpm --filter @taskorg/create-taskorg-plugin build
node packages/plugins/create-taskorg-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@taskorg/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@taskorg/plugin-sdk` from the local TaskOrg checkout into a `.taskorg-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into TaskOrg from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@taskorg/plugin-sdk/ui`
