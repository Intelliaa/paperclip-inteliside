import type { FinanceByBiller } from "@taskorg/shared";
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1 text-xs">
              {row.eventCount} evento{row.eventCount === 1 ? "" : "s"} en {row.kindCount} tipo{row.kindCount === 1 ? "" : "s"}
            </CardDescription>
          </div>
          <div className="text-right">
            <div className="text-lg font-semibold tabular-nums">{formatCents(row.netCents)}</div>
            <div className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">neto</div>
          </div>
        </div>
      </CardHeader>
