...
import { buildAgentMentionHref, buildProjectMentionHref, buildSkillMentionHref } from "@taskorg/shared";
...
describe("MarkdownBody", () => {
  ...
  it("renders agent, project, and skill mentions as chips", () => {
    const html = renderToStaticMarkup(
      <ThemeProvider>
        <MarkdownBody>
          {`[@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@TaskOrg App](${buildProjectMentionHref("project-456", "#336699")}) [/release-changelog](${buildSkillMentionHref("skill-789", "release-changelog")})`}
        </MarkdownBody>
      </ThemeProvider>,
    );
    ...
    expect(html).toContain("--taskorg-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--taskorg-mention-project-color:#336699");
