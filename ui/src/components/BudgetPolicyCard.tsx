...
import type { BudgetPolicySummary } from "@taskorg/shared";
...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? "Presupuesto de por vida" : "Presupuesto mensual UTC";
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Gastado</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `${summary.utilizationPercent}% del límite` : "Sin límite configurado"}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Presupuesto</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : "Desactivado"}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          Alerta suave al {summary.warnPercent}%{summary.paused && summary.pauseReason ? ` · pausa por ${summary.pauseReason}` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>Restante</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : "Sin límite"}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? "La ejecución está pausada para este proyecto hasta que se aumente el presupuesto o se cierre el incidente."
          : "Los heartbeats están pausados para este scope hasta que se aumente el presupuesto o se cierre el incidente."}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? "Guardando..." : summary.amount > 0 ? "Actualizar presupuesto" : "Establecer presupuesto"}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? "Pausado" : summary.status === "warning" ? "Advertencia" : summary.status === "hard_stop" ? "Límite alcanzado" : "Saludable"}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">Ingresa un monto en dólares válido y no negativo.</p>
        ) : null}
      </div>
    );
  }
