    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? "Guardando..." : "Guardar"}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">Identidad</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">Identidad</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label="Título" hint={help.title}>
              <DraftInput
                ...
                immediate
                className={inputClass}
                placeholder="ej. VP de Ingeniería"
              />
            </Field>
            <Field label="Reporta a" hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel="Elegir jefe…"
              />
            </Field>
            <Field label="Capacidades" hint={help.capabilities}>
              <MarkdownEditor
                ...
                placeholder="Describe lo que puede hacer este agente..."
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  El prompt template se envía en cada heartbeat. Mantenlo compacto y dinámico para evitar costos de tokens recurrentes.
                </div>
              </>
            )}
          </div>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          ...
          {showAdapterTestEnvironmentButton && (
            ...
            >
              {testEnvironment.isPending ? "Probando..." : "Probar entorno"}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label="Tipo de adapter" hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label="Directorio de trabajo (obsoleto)" hint={help.cwd}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                El prompt template se envía en cada heartbeat. Prefiere encuadre pequeño de tareas y variables como <code>{"{{ context.* }}"}</code> o <code>{"{{ run.* }}"}</code>; evita repetir instrucciones estáticas aquí.
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">Permisos y Configuración</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">Permisos y Configuración</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              <ModelDropdown
                ...
                emptyDetectHint="Sin modelo detectado. Selecciona o ingresa uno manualmente."
              />
              {fetchedModelsError && (
                <p className="text-xs text-destructive">
                  {fetchedModelsError instanceof Error
                    ? fetchedModelsError.message
                    : "Error al cargar modelos del adapter."}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        Codex puede rechazar el pensamiento `minimal` cuando la búsqueda está habilitada.
                      </p>
                    )}
                </>
              )}
                ...
                <>
                  <Field label="Bootstrap prompt (legacy)" hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      ...
                      placeholder="Prompt de configuración inicial opcional para la primera ejecución"
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    El bootstrap prompt es obsoleto y se eliminará en una versión futura. Considera mover este contenido al prompt template o archivo de instrucciones del agente.
                  </div>
                </>
              )}
              ...
              <Field label="Args extra (separados por coma)" hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder="ej. --verbose, --foo=bar"
                />
              </Field>

              <Field label="Variables de entorno" hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label="Tiempo límite (seg)" hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label="Período de gracia de interrupción (seg)" hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> Política de ejecución</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> Política de ejecución</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              ...
              numberLabel="sec"
              numberPrefix="Ejecutar heartbeat cada"
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title="Política de ejecución avanzada"
              bordered={cards}
            ...
            <div className="space-y-3">
              ...
              <Field label="Máx. ejecuciones concurrentes" hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const statusLabel =
    result.status === "pass" ? "Aprobado" : result.status === "warn" ? "Advertencias" : "Fallido";
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">Sugerencia: {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    const name = window.prompt("Nombre del secreto", suggested)?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : "Error al crear el secreto");
    }
  }
          ...
          <div key={i} className="flex items-center gap-1.5">
            ...
            {row.source === "secret" ? (
              <>
                ...
                >
                  <option value="">Seleccionar secreto...</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title="Crear secreto desde el valor actual"
                >
                  Nuevo
                </button>
              </>
              ...
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title="Guardar como secreto y reemplazar con referencia"
                >
                  Sellar
                </button>
              </>
        ...
        <PopoverTrigger asChild>
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? "Predeterminado" : required ? "Seleccionar modelo (requerido)" : "Seleccionar modelo")}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? "Buscar modelos... (escribe para crear)" : "Buscar modelos..."}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? "Detectando..." : detectedModel ? (detectModelLabel?.replace(/^Detectar\b/, "Re-detectar") ?? "Re-detectar desde config") : (detectModelLabel ?? "Detectar desde config")}
            </button>
          )}
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? "Sin modelo detectado. Ingresa un proveedor/modelo manualmente.")
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label="Esfuerzo de razonamiento" hint={help.thinkingEffort}>
