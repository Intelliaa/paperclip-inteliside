...
/**
 * Filter .taskorg.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterTaskOrgYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://taskorg.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm taskorg company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [TaskOrg](https://taskorg.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [TaskOrg](https://taskorg.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
function ExportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message="Selecciona un archivo para previsualizar su contenido." />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            La vista previa de archivos binarios no está disponible para este tipo de archivo.
          </div>
        )}
      </div>
    </div>
...
export function CompanyExport() {
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : "Error al cargar los datos de exportación.",
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        ...
        body: `${resultCheckedFiles.size} archivo${resultCheckedFiles.size === 1 ? "" : "s"} exportado${resultCheckedFiles.size === 1 ? "" : "s"} como ${result.rootPath}.zip`,
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: "Error de exportación",
        body: err instanceof Error ? err.message : "Error al construir el paquete de exportación.",
      });
    },
  });
  ...
  // Recompute .taskorg.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .taskorg.yaml
    const yamlPath = exportData.taskorgExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterTaskOrgYaml(exportData.files[yamlPath], checkedFiles);
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message="Selecciona una empresa para exportar." />;
  }
  ...
  if (!exportData) {
    return <EmptyState icon={Package} message="Cargando datos de exportación..." />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              Exportar {selectedCompany?.name ?? "Empresa"}
            </span>
            <span className="text-muted-foreground">
              {selectedCount} / {totalFiles} archivo{totalFiles === 1 ? "" : "s"} seleccionado{totalFiles === 1 ? "" : "s"}
            </span>
            {warnings.length > 0 && (
      ...
      <div className="grid h-[calc(100vh-12rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
        <aside className="flex flex-col border-r border-border overflow-hidden">
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h2 className="text-base font-semibold">Archivos del paquete</h2>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  Mostrar más tareas ({visibleTaskChildren} de {totalTaskChildren})
                </button>
              </div>
            )}
          </div>
