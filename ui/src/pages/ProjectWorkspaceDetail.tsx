...
function parseRuntimeConfigJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: "El JSON de servicios de ejecución debe ser un objeto JSON.",
      };
    }
...
function validateWorkspaceForm(form: WorkspaceFormState) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return "Los workspaces administrados remotamente requieren una referencia remota o URL de repositorio.";
    }
  } else if (!cwd && !repoUrl) {
    return "El workspace requiere al menos una ruta local o URL de repositorio.";
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return "La ruta local del workspace debe ser absoluta.";
  }
  ...
  if (repoUrl) {
    try {
      ...
      return "La URL del repositorio debe ser una URL válida.";
    }
  }
...
export function ProjectWorkspaceDetail() {
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : "Error al guardar el workspace.");
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : "Error al actualizar el workspace.");
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, action) => {
      ...
      setRuntimeActionMessage(
        action === "stop"
          ? "Servicios de ejecución detenidos."
          : action === "restart"
            ? "Servicios de ejecución reiniciados."
            : "Servicios de ejecución iniciados.",
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : "Error al controlar los servicios de ejecución.");
    },
  });

  if (projectQuery.isLoading) return <p className="text-sm text-muted-foreground">Cargando workspace…</p>;
  if (projectQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {projectQuery.error instanceof Error ? projectQuery.error.message : "Error al cargar el workspace"}
      </p>
    );
  }
  if (!project || !workspace || !form || !initialState) {
    return <p className="text-sm text-muted-foreground">Workspace no encontrado para este proyecto.</p>;
  }
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-xs text-muted-foreground">
          {workspace.isPrimary ? "Workspace principal" : "Workspace secundario"}
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
              ...
              {!workspace.isPrimary ? (
                ...
                <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
                  ...
                  Este es el workspace principal del código del proyecto.
                </div>
              )}
            </div>
            ...
            <div className="mt-4 grid gap-4">
              <Field label="Tipo de fuente" hint={sourceTypeDescription ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Ref del repositorio">
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Ref predeterminada">
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                ...
                <Field label="Clave de workspace compartido">
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Ref del workspace remoto">
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Comando de configuración" hint="Se ejecuta cuando este workspace necesita un bootstrap personalizado">
                  <textarea
                ...
                <Field label="Comando de limpieza" hint="Se ejecuta antes del desmontaje del workspace de ejecución a nivel de proyecto">
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <Field label="JSON de servicios de ejecución" hint="Servicios de ejecución predeterminados para este workspace. Los workspaces de ejecución heredan esta configuración a menos que establezcan una personalización. Si aún no conoces los comandos, pide a tu CEO que los configure por ti.">
                <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">Sin cambios pendientes.</p> : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Datos del workspace</div>
            ...
            <DetailRow label="Ruta local">
              <span className="break-all font-mono text-xs">{workspace.cwd ?? "Ninguna"}</span>
            </DetailRow>
            ...
            <DetailRow label="Ref predeterminada">{workspace.defaultRef ?? "Ninguna"}</DetailRow>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Servicios de ejecución</div>
                <h2 className="text-lg font-semibold">Servicios adjuntos</h2>
                <p className="text-sm text-muted-foreground">
                  Servicios compartidos para este workspace del proyecto. Los workspaces de ejecución heredan esta configuración a menos que la sobrescriban.
                </p>
              </div>
            ...
            {workspace.runtimeServices && workspace.runtimeServices.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="space-y-1 text-xs text-muted-foreground">
                          ...
                          <div>{service.command ?? "Sin comando registrado"}</div>
              ...
              <p className="text-sm text-muted-foreground">
                {workspace.runtimeConfig?.workspaceRuntime
                  ? "No hay servicios de ejecución corriendo actualmente para este workspace."
                  : "Aún no se ha configurado un servicio de ejecución predeterminado para este workspace."}
              </p>
            )}
          </div>
        </div>
