...
import { AGENT_ADAPTER_TYPES } from "@taskorg/shared";
import type { AgentAdapterType, JoinRequest } from "@taskorg/shared";
...
export function InviteLandingPage() {
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      if (!invite) throw new Error("Invitación no disponible");
  ...
  if (!token) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">Token de invitación inválido.</div>;
  }

  if (inviteQuery.isLoading || healthQuery.isLoading || sessionQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">Cargando invitación...</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Esta invitación puede haber expirado, sido revocada o ya utilizada.
          </p>
        </div>
      </div>
    );
  ...
  if (result?.kind === "bootstrap") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">Bootstrap completado</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            El primer administrador de la instancia ya está configurado. Puedes continuar al tablero.
          </p>
          <Button asChild className="mt-4">
            <Link to="/">Abrir tablero</Link>
          </Button>
        </div>
      </div>
    );
  ...
  if (result?.kind === "join") {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">Solicitud de unión enviada</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Tu solicitud está pendiente de aprobación del administrador. No tendrás acceso hasta que sea aprobada.
          </p>
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
            ID de solicitud: <span className="font-mono">{payload.id}</span>
          </div>
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">Secreto de reclamo de un solo uso (guárdalo ahora)</p>
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">Bootstrap de skill de TaskOrg</p>
              ...
              {onboardingInstallPath && <p className="font-mono break-all">Instalar en {onboardingInstallPath}</p>}
            </div>
          )}
          {(onboardingTextUrl || onboardingTextPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">Texto de incorporación legible por el agente</p>
          ...
          {diagnostics.length > 0 && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">Diagnósticos de conectividad</p>
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo"
            ? "Inicializa tu instancia de TaskOrg"
            : companyName
              ? `Unirse a ${companyName}`
              : "Unirse a esta compañía de TaskOrg"}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {invite.inviteType !== "bootstrap_ceo" && companyName
            ? `Fuiste invitado a unirte a ${companyName}. `
            : null}
          La invitación expira el {dateTime(invite.expiresAt)}.
        </p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                Unirse como {type === "human" ? "humano" : "agente"}
              </button>
            ))}
          </div>
        )}
        ...
        {joinType === "agent" && invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-4 space-y-3">
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">Nombre del agente</span>
              <input
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">Tipo de adapter</span>
              <select
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                value={adapterType}
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {getAdapterLabel(type)}{!ENABLED_INVITE_ADAPTERS.has(type) ? " (Próximamente)" : ""}
                  </option>
                ))}
              </select>
            </label>
        ...
        {requiresAuthForHuman && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            Inicia sesión o crea una cuenta antes de enviar una solicitud de unión humana.
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>Iniciar sesión / Crear cuenta</Link>
              </Button>
            </div>
          </div>
        )}
        ...
        >
          {acceptMutation.isPending
            ? "Enviando…"
            : invite.inviteType === "bootstrap_ceo"
              ? "Aceptar invitación de bootstrap"
