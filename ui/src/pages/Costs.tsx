  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">Libro de finanzas</CardTitle>
        <CardDescription>
          Cargos a nivel de cuenta que no corresponden a una sola solicitud de inferencia.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={`${eventCount} evento${eventCount === 1 ? "" : "s"} en el rango`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label="Créditos"
          value={formatCents(creditCents)}
          subtitle="Reembolsos, compensaciones y devoluciones de crédito"
          icon={ArrowDownLeft}
        />
        <MetricTile
          label="Neto"
          value={formatCents(netCents)}
          subtitle="Débito menos crédito para el período seleccionado"
          icon={ReceiptText}
        />
        <MetricTile
          label="Estimado"
          value={formatCents(estimatedDebitCents)}
          subtitle="Débitos estimados que aún no son definitivos en facturación"
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>Todos los facturadores</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="Selecciona una empresa para ver los costos." />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  Gasto de inferencia, tarifas de plataforma, créditos y ventanas de cuota en vivo.
                </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label="Gasto de inferencia"
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={`${formatTokens(inferenceTokenTotal)} tokens en eventos por solicitud`}
              icon={DollarSign}
            />
            <MetricTile
              label="Presupuesto"
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} agentes pausados · ${budgetData?.pausedProjectCount ?? 0} proyectos pausados`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
            ...
            <MetricTile
              ...
              subtitle={`${formatCents(financeData?.summary.debitCents ?? 0)} débitos · ${formatCents(financeData?.summary.creditCents ?? 0)} créditos`}
              icon={ReceiptText}
            />
            <MetricTile
              label="Eventos financieros"
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={`${formatCents(financeData?.summary.estimatedDebitCents ?? 0)} estimados en el rango`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="billers">Facturadores</TabsTrigger>
          <TabsTrigger value="finance">Finanzas</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">Selecciona una fecha de inicio y fin para cargar los datos.</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">Registro de inferencia</CardTitle>
                    <CardDescription>
                      Gasto de inferencia por solicitud para el período seleccionado.
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? `Presupuesto ${formatCents(spendData.summary.budgetCents)}`
                            : "Presupuesto ilimitado"}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Uso</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% del presupuesto mensual consumido en este rango.
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>Lo que consumió cada agente en el período seleccionado.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">Aún no hay eventos de costo.</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  entrada {formatTokens(row.inputTokens + row.cachedInputTokens)} · salida {formatTokens(row.outputTokens)}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    ...
                                    {row.subscriptionRunCount > 0
                                      ? `${row.subscriptionRunCount} suscripción`
                                      : "0 suscripción"}
                                  </div>
                                ) : null}
                              </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>Costos de ejecución atribuidos a través de tareas vinculadas a proyectos.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Aún no hay costos de ejecución atribuidos a proyectos.</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? "Sin atribuir"}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="Aún no hay eventos financieros. Agrega cargos a nivel de cuenta cuando lleguen facturas o créditos." />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">Panel de control de presupuesto</CardTitle>
                  <CardDescription>
                    Límites de gasto estrictos para agentes y proyectos. La cuota de suscripción del proveedor se mantiene separada y aparece en Proveedores.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle="Cruces de umbral suave o duro abiertos"
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label="Aprobaciones pendientes"
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle="Aprobaciones de excepción de presupuesto esperando acción del directorio"
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label="Agentes pausados"
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle="Heartbeats de agentes bloqueados por presupuesto"
                    icon={Coins}
                  />
                  <MetricTile
                    label="Proyectos pausados"
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle="Ejecución de proyectos bloqueada por presupuesto"
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Resuelve los bloqueos aquí aumentando el presupuesto o manteniendo explícitamente el alcance pausado.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">presupuestos de {scopeType}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "Política mensual para toda la empresa."
                            : scopeType === "agent"
                              ? "Políticas de gasto mensual recurrentes para agentes individuales."
                              : "Políticas de gasto de por vida para proyectos vinculados a ejecución."}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      Aún no hay políticas de presupuesto. Configura presupuestos de agentes y proyectos desde sus páginas de detalle, o usa el control de presupuesto mensual de la empresa.
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Sin eventos de costo en este período.</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Sin eventos facturables en este período.</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">Por facturador</CardTitle>
                      <CardDescription>Eventos financieros a nivel de cuenta agrupados por quién los cobró o acreditó.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Aún no hay eventos financieros.</p>
                      ) : (
