...
export function BoardClaimPage() {
  ...
  if (!token || !code) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">URL de reclamo de board inválida.</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {statusQuery.error instanceof Error ? statusQuery.error.message : "El desafío es inválido o ha expirado."}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Inicia sesión o crea una cuenta, luego vuelve a esta página para reclamar la propiedad del Board.
          </p>
          <Button asChild className="mt-4">
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Esto promoverá tu usuario a administrador de instancia y migrará el acceso de propiedad de la empresa desde el modo local de confianza.
        </p>

        {claimMutation.error && (
          <p className="mt-3 text-sm text-destructive">
            {claimMutation.error instanceof Error ? claimMutation.error.message : "Error al reclamar la propiedad del board"}
          </p>
        )}
