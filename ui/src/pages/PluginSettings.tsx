...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? "Detalles del Plugin" },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">Cargando detalles del plugin...</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || "Sin descripción proporcionada.";
            ...
            <section className="space-y-5">
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">Categorías</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">Ninguna</span>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_320px]">
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Proceso worker, trabajos programados y entregas de webhook
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        ...
                        {dashboardData.worker ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">No hay proceso worker registrado.</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">Aún no se han registrado ejecuciones.</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">Aún no se han registrado entregas de webhook.</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        Última verificación: {new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      Los diagnósticos de ejecución no están disponibles en este momento.
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Últimas {recentLogs.length} entradas de registro</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">Verificando salud...</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>Las verificaciones de salud se ejecutan cuando el plugin está listo.</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">No se solicitaron permisos especiales.</p>
                  )}
                </CardContent>
              </Card>
            </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || "Error al guardar la configuración." });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: "Prueba de configuración exitosa." });
      } else {
        setTestResult({ type: "error", text: result.message || "Prueba de configuración fallida." });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || "Prueba de configuración fallida." });
    },
  });
