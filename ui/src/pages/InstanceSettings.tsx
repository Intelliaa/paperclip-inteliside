...
import type { InstanceSchedulerHeartbeatAgent } from "@taskorg/shared";
...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "Error al actualizar el heartbeat.");
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? `Error al deshabilitar 1 heartbeat de temporizador: ${detail}`
            : `Error al deshabilitar ${failures.length} de ${enabled.length} heartbeats de temporizador. Primer error: ${detail}`,
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "Error al deshabilitar todos los heartbeats.");
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">Cargando heartbeats del programador...</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : "Error al cargar los heartbeats del programador."}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Heartbeats del programador</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          Agentes con un heartbeat de temporizador habilitado en todas tus compañías.
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> activos</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> deshabilitados</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? "compañía" : "compañías"}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              ...
              if (!window.confirm(`¿Deshabilitar heartbeats de temporizador para los ${enabledCount} ${noun} habilitados?`)) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? "Deshabilitando..." : "Deshabilitar todos"}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message="Ningún heartbeat del programador coincide con los criterios actuales."
        />
      ) : (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title="Configuración completa del agente"
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? "Deshabilitar heartbeat de temporizador" : "Habilitar heartbeat de temporizador"}
                          </Button>
                        </span>
                      </div>
                    );
