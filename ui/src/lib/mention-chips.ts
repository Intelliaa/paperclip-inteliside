...
import { parseAgentMentionHref, parseProjectMentionHref, parseSkillMentionHref } from "@taskorg/shared";
...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--taskorg-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--taskorg-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("taskorg-mention-chip", `taskorg-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("taskorg-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    "taskorg-mention-chip",
    "taskorg-mention-chip--agent",
    "taskorg-mention-chip--project",
    "taskorg-mention-chip--skill",
    "taskorg-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--taskorg-mention-project-color");
  element.style.removeProperty("--taskorg-mention-icon-mask");
}
