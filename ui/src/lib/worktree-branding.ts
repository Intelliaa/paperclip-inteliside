...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("taskorg-worktree-enabled") !== "true") return null;

  const name = readMetaContent("taskorg-worktree-name");
  const color = normalizeHexColor(readMetaContent("taskorg-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("taskorg-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
