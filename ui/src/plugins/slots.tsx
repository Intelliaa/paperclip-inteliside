 ...
 *
 * Plugin bundles are built with `external: ["@taskorg/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__taskorgPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__taskorgPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__taskorgPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__taskorgPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@taskorg/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@taskorg/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@taskorg/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@taskorg/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@taskorg/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__taskorgPluginBridge__) {
 ...
 * self-register via `window.taskorgPlugins.registerReactComponent()`.
 ...
 * of `@taskorg/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
