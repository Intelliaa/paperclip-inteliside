...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@taskorg/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@taskorg/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@taskorg/adapter-utils").TranscriptEntry[];
