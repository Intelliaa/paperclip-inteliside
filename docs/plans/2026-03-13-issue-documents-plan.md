...
Add first-class **documents** to TaskOrg as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by TaskOrg workflow/docs.
...
Update the TaskOrg agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/taskorg/SKILL.md`
