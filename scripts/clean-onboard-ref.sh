...
Environment overrides:
  ...
  PC_DATA=/tmp/data           TaskOrg data dir to use
  TASKORG_HOST=127.0.0.1    Host passed to the onboarded server
  TASKORG_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/taskorg-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/taskorg-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export TASKORG_HOST="${TASKORG_HOST:-127.0.0.1}"
export TASKORG_PORT="${TASKORG_PORT:-3100}"
export TASKORG_OPEN_ON_LISTEN="${TASKORG_OPEN_ON_LISTEN:-false}"
...
echo "TASKORG_HOST: $TASKORG_HOST"
echo "TASKORG_PORT: $TASKORG_PORT"
...
env \
  ...
  HOST="$TASKORG_HOST" \
  PORT="$TASKORG_PORT" \
  TASKORG_OPEN_ON_LISTEN="$TASKORG_OPEN_ON_LISTEN" \
  pnpm taskorg onboard --yes --data-dir "$PC_DATA"
