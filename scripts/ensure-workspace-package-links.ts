...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".taskorg", "dist", "node_modules"]);
...
async function ensureServerWorkspaceLinksCurrent() {
  ...
  console.log("[taskorg] detected stale workspace package links for server; relinking dependencies...");
  for (const mismatch of mismatches) {
    console.log(
      `[taskorg]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
