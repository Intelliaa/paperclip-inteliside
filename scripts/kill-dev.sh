#!/usr/bin/env bash
#
# Kill all local TaskOrg dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all taskorg dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/taskorg(-[^/]+)?/' | grep node | grep -v grep || true)

candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.taskorg/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.taskorg/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.taskorg/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/taskorg*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.taskorg/instances/*/db/postmaster.pid
    "$sibling_root"/.taskorg/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No TaskOrg dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} TaskOrg dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to TaskOrg node processes..."
