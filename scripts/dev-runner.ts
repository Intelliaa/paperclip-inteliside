...
const devServerStatusFilePath = path.join(repoRoot, ".taskorg", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".taskorg/dev-server-status.json",
]);
...
const env: NodeJS.ProcessEnv = {
  ...process.env,
  TASKORG_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.TASKORG_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.TASKORG_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.TASKORG_MIGRATION_PROMPT ??= "never";
  env.TASKORG_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.TASKORG_DEPLOYMENT_MODE = "authenticated";
  env.TASKORG_DEPLOYMENT_EXPOSURE = "private";
  env.TASKORG_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[taskorg] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[taskorg] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[taskorg] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@taskorg/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[taskorg] Command failed with code ${status.code}: pnpm --filter @taskorg/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[taskorg] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.TASKORG_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[taskorg] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[taskorg] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@taskorg/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[taskorg] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@taskorg/server", serverScript, ...forwardedArgs],
