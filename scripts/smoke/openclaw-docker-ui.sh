...
TASKORG_HOST_PORT="${TASKORG_HOST_PORT:-3100}"
TASKORG_HOST_FROM_CONTAINER="${TASKORG_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.taskorg-openclaw.override.yml"
...
detect_taskorg_base_url() {
  ...
  for candidate in "$TASKORG_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${TASKORG_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${TASKORG_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
taskorg_base_url="$(detect_taskorg_base_url || true)"
...
TaskOrg URL for OpenClaw container:
EOF
  if [[ -n "$taskorg_base_url" ]]; then
    cat <<EOF
  $taskorg_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${TASKORG_HOST_PORT}
  (Do not use http://127.0.0.1:${TASKORG_HOST_PORT} inside the container.)
  If TaskOrg rejects the host, run on host machine:
    pnpm taskorg allowed-hostname host.docker.internal
  Then restart TaskOrg and re-run this script.
EOF
  fi
  cat <<EOF
