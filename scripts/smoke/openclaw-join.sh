...
COMPANY_ID="${COMPANY_ID:-${TASKORG_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-taskorg-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-taskorg-openclaw-smoke}"
...
Provide one of:
  TASKORG_AUTH_HEADER=\"Bearer <board-token>\"
  TASKORG_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "TaskOrg OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.taskorg.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
