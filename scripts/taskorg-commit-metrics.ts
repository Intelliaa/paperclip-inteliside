...
const DEFAULT_QUERY = "\"Co-Authored-By: TaskOrg <noreply@taskorg.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/taskorg-commit-metrics-cache.json");
...
const TASKORG_EMAIL = "noreply@taskorg.ing";
const TASKORG_NAME = "taskorg";
...
function printHelp() {
  console.log(`Usage: tsx scripts/taskorg-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === TASKORG_EMAIL) || (displayName && displayName.toLowerCase() === TASKORG_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("TaskOrg commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "taskorg-commit-metrics",
