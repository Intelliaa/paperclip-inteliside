#!/usr/bin/env node
/**
 * Screenshot utility for TaskOrg UI.
 *
 * Reads the board token from ~/.taskorg/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/taskorg-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".taskorg/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.taskorg/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the TaskOrg origin only
